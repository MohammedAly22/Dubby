...
(preferring commas), then fold tiny leftovers into their neighbours.
"""
...
def _chunk_from_words(words: List[Dict[str, Any]]) -> Dict[str, Any]:
    return {
        "id": new_id(),
...
def build_chunks(
        ...
        for w in stream:
            if current:
                ...
                if gap > max_gap or (SENTENCE_END.search(prev["text"]) and cur_dur >= min_seconds):
                    chunks.append(_chunk_from_words(current))
                    current = []
                    ...
                    if split_at and current[split_at - 1]["end"] - current[0]["start"] >= min_seconds:
                        chunks.append(_chunk_from_words(current[:split_at]))
    ...
    flush_stream()
    return _merge_tiny(chunks, min_seconds, max_seconds, max_gap)


def _merge_tiny(chunks: List[Dict[str, Any]], min_seconds: float, max_seconds: float, max_gap: float) -> List[Dict[str, Any]]:
    if not chunks:
        return chunks
    ...
    for c in chunks[1:]:
        ...
        if (prev_short or cur_short) and close and fits and not SENTENCE_END.search(prev["text"]) or (
            cur_short and close and fits and len(c["text"].split()) <= 2
        ):
            prev["end"] = c["end"]
...
def split_segment_words(seg: Dict[str, Any], word_index: int) -> List[Dict[str, Any]]:
    ...
    return [_chunk_from_words(words[:word_index]), _chunk_from_words(words[word_index:])]
