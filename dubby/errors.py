...
def explain(error: Union[BaseException, str], family: Optional[str] = None) -> Explained:
    ...
    if "resource_exhausted" in low or ("429" in low and ("gemini" in low or "quota" in low or "rate" in low)):
        daily = "daily" in low or "per day" in low or "perday" in low.replace("_", "") or "per_day" in low
        return Explained(
            ...
            + ("Wait for the daily reset (the clips generated so far are kept), or pick another Gemini model yourself. "
